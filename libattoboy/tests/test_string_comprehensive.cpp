...
void atto_main() {
  EnableLoggingToFile(ATTO_TEXT("test_string_comprehensive.log"), true);
  ...
  // getPositionOf()
  {
    ...
    ASSERT_EQ(s.getPositionOf(String(ATTO_TEXT("world"))), 6);
    ASSERT_EQ(s.getPositionOf(String(ATTO_TEXT("hello"))), 0);
    ASSERT_EQ(s.getPositionOf(String(ATTO_TEXT("xyz"))), -1); // Not found
    ASSERT_EQ(s.getPositionOf(String(ATTO_TEXT(""))), 0);  // Empty string at 0
    ASSERT_EQ(s.getPositionOf(String(ATTO_TEXT(" "))), 5); // Space between
    // UTF-8 test
    String utf8(ATTO_TEXT("héllo wörld"));
    ASSERT_EQ(utf8.getPositionOf(String(ATTO_TEXT("wörld"))),
              6); // h,e,l,l,o,  -> é is 1 char
    // "héllo " is h,é,l,l,o,space -> 6 chars
    Log(ATTO_TEXT("getPositionOf(): passed"));
  }
  ...
  // replace()
  {
    String s(ATTO_TEXT("hello world"));
    String result =
        s.replace(String(ATTO_TEXT("world")), String(ATTO_TEXT("there")));
