...
void atto_main() {
  ...
  // compress()
  {
    Buffer original;
    original.append(
        String("This is a test string that should compress well. "));
  ...
  // toString() - UTF-8
  {
    ...
    REGISTER_TESTED(Buffer_toString_utf8);
    ASSERT_FALSE(s.isEmpty());
    ASSERT_TRUE(s.contains(String("hello")));
    Log("toString(utf-8): passed");
  }

  // toString() - ANSI (basic test)
  {
    Buffer b;
    b.append(String("hello"));
    String s = b.toString("ansi");
    REGISTER_TESTED(Buffer_toString_ansi);
    ASSERT_FALSE(s.isEmpty()); // Should create some string
    Log("toString(ansi): passed");
  }
  ...
  // - Buffer_at, Buffer_operator_brackets, Buffer_set (byte-level access not in
  // public API)
  // - Buffer_append_byte, Buffer_prepend_byte, Buffer_insert_byte (not in
  // public API)
