...
Папка **docs/html/** (включена в ваш `dist`) содержит полную API-документацию для каждого класса и функции в attoboy. Откройте `docs/html/index.html` в вашем веб-браузере, чтобы просмотреть полную справку.
